...
<head>
    ...
    <style>
        *{font-family:'Inter',sans-serif;}
        ...
        .auth-card{background:#fff;border-radius:20px;box-shadow:0 20px 60px rgba(79,70,229,.25);width:100%;max-width:420px;overflow:hidden;}
        ...
        .form-control,.form-check-input{border-radius:8px;border:1.5px solid #e2e8f0;font-size:.9rem;padding:.65rem .9rem;transition:border-color .2s,box-shadow .2s;}
        ...
        .input-icon-group .bi{position:absolute;left:.9rem;top:50%;transform:translateY(-50%);color:#94a3b8;font-size:1rem;}
...
<div class="auth-card">
    <div class="auth-header">
        ...
        <p class="mb-0 opacity-75" style="font-size:.88rem;">Masuk ke akun Anda</p>
    </div>
    <div class="auth-body">
        @if(session('status'))
            <div class="alert alert-success mb-3" style="border-radius:10px;font-size:.88rem;">{{ session('status') }}</div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="d-flex align-items-center justify-content-between mb-4">
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="remember" style="font-size:.88rem;">Ingat saya</label>
                </div>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" style="font-size:.88rem;color:#4f46e5;">Lupa password?</a>
                @endif
            </div>
        ...
        <p class="text-center mb-0" style="font-size:.88rem;color:#64748b;">
            Belum punya akun?
            <a href="{{ route('register') }}" style="color:#4f46e5;font-weight:600;">Daftar sekarang</a>
        </p>
